...
makefunctionfiles <- function(dir_data_remote_DStats,Stat.Family,Sub.Stat.Name,sep){
   require(Greatrex.Functions)
   #------------------------------------------------------------------------------------------
   # This then sets up your folder structure for your output data,
   # based on the folder names you chose
   #------------------------------------------------------------------------------------------
   dir_data_remote_DStat_family    <- (paste(dir_data_remote_DStats,Stat.Family,sep=sep))
   dir_data_remote_DStat_statistic <- (paste(dir_data_remote_DStat_family,Sub.Stat.Name,sep=sep))
   conditionalcreate(dir_data_remote_DStats)
   conditionalcreate(dir_data_remote_DStat_family)
   conditionalcreate(dir_data_remote_DStat_statistic)
}
...
phonebook <- function(ProductTime,ProductType,ProductFamily,...){
   # if you had a dataframe, you could have mulitple data types in here..
   #------------------------------------------------------------------------------------------
   ...
   # instead of output 1-4 have data frame?
