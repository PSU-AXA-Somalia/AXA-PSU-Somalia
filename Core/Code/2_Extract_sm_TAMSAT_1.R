...
#=================================================================================
 ...
 CreateTAMSAT_SM <- function(f,dir_data_in,OutFolder_Route,StemOut,date.list,file_list.in,
    ...
    subfolder <- "smcl_1"
    dir_data_out <- paste(OutFolder_Route,paste(StemOut,subfolder,"Geo",sep="_"),substr(date.list[f],1,4),sep=sep)
    ...
    if(continue == TRUE){
       ...
       dir_data_out <- paste(OutFolder_Route,paste(StemOut,subfolder,"Geo",sep="_"),substr(date.list[f],1,4),sep=sep)
       if(!dir.exists(dir_data_out)){dir.create(dir_data_out)}
       ...
       a<-try(suppressMessages(suppressWarnings(file.remove(paste(file_name.out,".aux.json",sep="")))))
       a<-try(suppressMessages(suppressWarnings(file.remove(paste(file_name.out,".tif.aux",sep="")))))
 ...
 #---------------------------------------------------------------------------------
  ...
  myres <- foreach(f = 1:length(file_list.in)) %dopar%  CreateTAMSAT_SM(f,dir_data_in,OutFolder_Route,StemOut,date.list,file_list.in,
                                                      LayerThickness_1,LayerThickness_2,LayerThickness_3,LayerThickness_4,
                                                      globalcrs,dataoverwrite)
