...
Output_datasetlist <- Daily_datasetlist

if(verbose %in% c(TRUE,"Limited")){message(paste("     Checking these datasets"))}
if(verbose %in% c(TRUE,"Limited")){print(Daily_datasetlist[,1:6])}
...
for(n_data in 1:nrow(Daily_datasetlist)){
   ...
   #-----------------------------------------------------------------------------
   # If zero data available, delete the subfolder and move on
   #-----------------------------------------------------------------------------
   if(length(data_available) > 0){
      ...
      length(which(fulldatelist$missing == TRUE))>0
      
      #-----------------------------------------------------------------------------
      # If there are missing days move forwards
      #-----------------------------------------------------------------------------
      if((length(which(fulldatelist$missing == TRUE))>0)&(length(which(fulldatelist$missing == TRUE))<100)){
         ...
         if(length(which(fulldatelist$missing == TRUE))>=100){
            if(verbose %in% c(TRUE,"Limited")){message(paste("           Over 100 missing data files. Moving on. Please use Step 2 instead"))}
         }
         if(length(which(fulldatelist$missing == TRUE)) <= 0){
            if(verbose %in% c(TRUE,"Limited")){message(paste("           Zero missing data"))}
         }      
      }
   }else{
      # no data available
      if(verbose %in% c(TRUE,"Limited")){message(paste("           No data available. Deleting subfolder. Rerun step 2"))}
      a <- try(suppressWarnings(suppressMessages(file.remove(list.files(geo_stem,recursive=TRUE,full.names=TRUE,include.dirs = TRUE)))))
      a <- try(suppressWarnings(suppressMessages(unlink(geo_stem,recursive = TRUE))))
      Output_datasetlist <- Output_datasetlist[-which(Output_datasetlist$Dataset %in% dataset),]
   }
}  

Daily_datasetlist <- Output_datasetlist
