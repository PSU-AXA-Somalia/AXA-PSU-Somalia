...
#================================================================================================================
  ...
  #---------------------------------------------------------------------------------
    ...
    if (any(installed_packages == FALSE)) {
       ...
       install.packages(cranpackages$Package[!installed_packages])
    }
    # Ones from Github
    ...
    if (any(remotes_installed_packages == FALSE)) {
       ...
       for(n in 1:nrow(remotes_missing)){
          ...
          remotes::install_github(paste(remotes_missing$Github[n],remotes_missing$Package[n],sep="/"),force=TRUE)
       }
    }
    ...
    if(updatepackage){ 
       ...
       for(n in 1:nrow(cranpackages)){
          if((package_needsupdating(cranpackages$Package[n],cranpackages$Version[n]))){
             ...
             install.packages(cranpackages$Package[n])
          }
       }
       #Github
       for(n in 1:nrow(githubpackages)){
          if((package_needsupdating(githubpackages$Package[n],githubpackages$Version[n]))){
             ...
             remotes::install_github(paste(githubpackages$Github[n],githubpackages$Package[n],sep="/"),force=TRUE)
          }
       }       
    }
    ...
    installed_packages <- cranpackages$Package %in% rownames(installed.packages())
    if (any(installed_packages == FALSE)) {
       stop(paste("CRAN Package instal failed:",cranpackages[which(installed_packages==FALSE)],"\n"))
    }
    installed_remote_packages <- githubpackages$Package %in% rownames(installed.packages())
    if (any(installed_remote_packages == FALSE)) {
  ...
  packages  <- c(cranpackages$Package,githubpackages$Package)
  out <- lapply(packages, function(x) suppressPackageStartupMessages(library(x, character.only = TRUE)))
  if(verbose){message(paste("           Package loaded:",packages,"\n"))}
